...
- **挙動**: 
  ...
  - **UI要素**:
    ...
    - 「保存」ボタン
    - 「キャンセル」ボタン
  - **挙動**: 「保存」ボタンを押すと、変更内容をDBへ保存する。 「キャンセル」ボタンを押すと、4.3.1へ戻る。

  ### 4.3.4 タスクの削除ボタン
  - **機能**: 選択されたタスクの削除（非表示）を可能とする。
  - **UI要素**:
    - 「削除」ボタン
  - **挙動**: 「削除」ボタンを押すと、「『課題名』を削除しますか？」というモーダルを表示し、ユーザーの同意を得た場合はDBの `isDeleted` フラグを `true` に更新し、一覧から非表示にする（データは物理的に削除しない）。得られなければ4.3.1へ戻る。
