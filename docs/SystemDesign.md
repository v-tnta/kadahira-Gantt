...
- **挙動**: Stop時に `durationSeconds` を計算する。計算は「終了時刻 - 開始時刻」の実差分を用いて算出し、正確な時間を記録する。サブタスク名が入力されていた場合は、DBへ保存する。サブタスク名が入力されていなかった場合は、サブタスク名を入力するように促すモーダルを表示して、その後DBへ保存する。
